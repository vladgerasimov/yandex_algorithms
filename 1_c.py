...
8<код><номер> <номер> где <номер> — это семь цифр, а <код> — это три цифры или три цифры в круглых скобках. Если код
не указан, то считается, что он равен 495. Кроме того, в записи телефонного номера может стоять знак “-” между любыми
...
import re
target_num = re.sub(r'[()-]', '', input()).replace('+7', '8')
if len(target_num) != 11:
    target_num = '8495' + target_num
numbers = []
for _ in range(3):
    numbers.append(re.sub(r'[()-]', '', input()).replace('+7', '8'))

for num in numbers:
    if len(num) != len(target_num):
        num = '8495' + num
    if num == target_num:
        print('YES')
    else:
